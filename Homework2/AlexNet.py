...
class AlexNet(nn.Module):
    def __init__(self):
        ...
        # Note: AlexNet does not use batch norm, this is here for other networks that do use batch norm
        DISABLE_BATCHNORM:bool = True
        batchNormLayer:nn.Module = nn.BatchNorm1d(num_features=4096)
        if DISABLE_BATCHNORM: batchNormLayer:nn.Module = nn.Identity()
        
        self.model = nn.Sequential(
            self._getConvBlock(in_channels=3,   out_channels=96,  # 3 -> 96
                               disablePool=True, 
                               disableBatchNorm=DISABLE_BATCHNORM, disableDropout=DISABLE_DROPOUT),   # Out: 32 x 32
            
            self._getConvBlock(in_channels=96,  out_channels=256, # 96 -> 256
                               disablePool=True, 
                               disableBatchNorm=DISABLE_BATCHNORM, disableDropout=DISABLE_DROPOUT),   # Out: 32 x 32
            
            self._getConvBlock(in_channels=256, out_channels=384, # 256 -> 384
            ...
            self._getConvBlock(in_channels=384, out_channels=256,  # 384 -> 256
                               poolKernel=2, poolStride=2, poolPadding=0, 
                               disableBatchNorm=DISABLE_BATCHNORM, disableDropout=DISABLE_DROPOUT),   # Out: 8 x 8 (Pooling => 1/2)
            
            self._getConvBlock(in_channels=256, out_channels=256,  # 256 -> 256
                               poolKernel=2, poolStride=2, poolPadding=0, 
                               disableBatchNorm=DISABLE_BATCHNORM, disableDropout=DISABLE_DROPOUT),   # Out: 4 x 4 (Pooling => 1/2)
            
            nn.Flatten(),
            
            nn.Linear(in_features=4*4*256, out_features=4096), # 4096 -> 4096
            batchNormLayer,
            nn.ReLU(),
            dropoutLayer,
    ...
    def _getConvBlock(self, in_channels:int, 
        ...
        if disablePool:
            poolLayer:nn.Identity = nn.Identity()
        
        if disableBatchNorm:
            batchNormLayer:nn.Identity = nn.Identity()
        
        if disableDropout:
            dropoutLayer:nn.Identity = nn.Identity()
...
with torch.inference_mode():
    ...
    print(input)
    print(model.model(input).argmax())
    print(model(input).argmax())

# %%
# ===============================================================================================================
#                                                   Load Model
# ===============================================================================================================
# model = torch.load('Saved_Models/AlexNet_baseline_CIFAR10.pt')
...
while not interrupted and ((epochIterator < EPOCHS or EPOCHS == -1) or trainEpochAccuracy < testEpochAccuracy + max(0, min(0, 99 - testEpochAccuracy)) or testEpochAccuracy < 50):
    ...
    for X_train_batch_imgs, Y_train_batch_labels in train_loader:
        ...
        if torch.isnan(trainBatchLoss).any():
        ...
        totalTrainLossInEpoch += trainBatchLoss.item()
    ...
    with torch.inference_mode():
        ...
        for X_test_batch_imgs, Y_test_batch_labels in test_loader:
            ...
            testBatchLoss = Loss_Function(Y_test_pred_imgs_logits, Y_test_batch_labels.type(torch.int64))
    
            # torch.cuda.empty_cache()
            # torch.cuda.synchronize()
            numCorrectInEpoch += torch.eq(Y_test_pred_imgs_labels.cpu(), Y_test_batch_labels.cpu()).sum().item()
            
            totalTestLossInEpoch += testBatchLoss.item()
...
#                                                   Save Model
# ===============================================================================================================
torch.save(model, 'Saved_Models/AlexNet_Dropout_CIFAR10.pt')
