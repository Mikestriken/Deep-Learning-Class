...
class Encoder(nn.Module):
    def __init__(self, embedding_size, hidden_size, padding_token = PADDING_TOKEN):
        ...
        self.rnn:nn.GRU = nn.GRU(input_size=embedding_size, hidden_size=hidden_size,
                                 batch_first=True, dropout=DROPOUT_PROB, num_layers=NUM_RNN_LAYERS, bidirectional=BI_DIRECTIONAL_ENCODER)
        
    
    def forward(self, X:torch.Tensor, Input_Hidden_State = None, X_sequence_lengths:torch.Tensor = None):
        # X.shape = (batch_size, seq_length, input_size)
        # hidden.shape = (num_layers * (1 + isBiDirectional), batch_size, hidden_size)
        # rnn_out.shape = (batch_size, seq_length, hidden_size * (1 + isBiDirectional))
...
class Decoder(nn.Module):
    def __init__(self, embedding_size, hidden_size, padding_token = PADDING_TOKEN):
        ...
        self.rnn:nn.GRU = nn.GRU(input_size=embedding_size, hidden_size=hidden_size,
                                 batch_first=True, dropout=DROPOUT_PROB, num_layers=NUM_RNN_LAYERS)
...
class BahdanauAttention(nn.Module):
    def __init__(self, hidden_size):
        super().__init__()
        self.Wa = nn.Linear(hidden_size * (1 + int(BI_DIRECTIONAL_ENCODER)), hidden_size, bias=False)  # Transform encoder outputs
        self.Ua = nn.Linear(hidden_size, hidden_size, bias=False)  # Transform decoder hidden state
        
        self.Va = nn.Linear(hidden_size, 1, bias=False)  # Compute alignment scores

    def forward(self, encoder_outputs, decoder_output):
        """
        encoder_outputs: (batch_size, seq_length, hidden_size)
        decoder_output: (batch_size, seq_length=1, hidden_size)
        """
        
        # Compute alignment scores
        alignment_scores = self.Va(torch.tanh(self.Wa(encoder_outputs) + self.Ua(decoder_output))) # (batch_size, seq_length, 1)

        # Compute attention weights
        attention_weights = torch.softmax(alignment_scores, dim=1)  # (batch_size, seq_length, 1)
        
        # Compute dot product similarity
        dot_product_similarity = encoder_outputs * attention_weights

        # Compute context vector as weighted sum of encoder outputs
        context_vector = torch.sum(dot_product_similarity, dim=1)  # (batch_size, hidden_size * (1 + isBiDirectional))
...
class Seq2Seq(nn.Module):
    def __init__(self):
        ...
        self.outputDenseLayer = nn.Sequential(
            ...
            nn.Linear(in_features=128*(2 + int(BI_DIRECTIONAL_ENCODER)), out_features=NUM_FRENCH_WORDS),
        )
    ...
    def forward(self, X:torch.Tensor, Y_Ground_Truth:torch.Tensor = None, X_sequence_lengths:torch.Tensor = None):
        ...
        encoder_outputs_sorted:torch.Tensor
        ...
        if X_isBatched:
            ...
            X_sorted = X[sortedIndices]
            
            # Pass input through encoder
            encoder_outputs_sorted, encoder_last_hidden_state = self.encoder(X=X_sorted, Input_Hidden_State=None, X_sequence_lengths=X_sequence_lengths_sorted)
            ...
            for sequence_index in range(X.shape[0]):
                ...
                encoder_output, encoder_last_hidden_state = self.encoder(X=input, Input_Hidden_State=encoder_last_hidden_state)
                
                encoder_outputs.append(encoder_output)
                
            encoder_outputs:torch.Tensor = torch.stack(encoder_outputs, dim=1).squeeze() # convert list to tensor
                
        # * Pass through decoder
        if X_isBatched and Teacher_Forcing_Enabled:
            # rnn_out.shape = (batch_size, seq_length=1, hidden_size * (1 + isBiDirectional))
            # hidden.shape = (num_layers * (1 + isBiDirectional), batch_size, hidden_size)
            ...
            initial_decoder_hidden_state = self.encoder2decoder_hidden_linear(encoder_last_hidden_state.permute(1, 2, 0)).tanh().permute(2, 0, 1)
            
            decoder_output_sorted, last_decoder_hidden_state = self.decoder(X=first_input, Input_Hidden_State=initial_decoder_hidden_state)
            
            context_vector = self.attention(encoder_outputs_sorted, decoder_output_sorted)
            dense_layer_input = torch.cat((decoder_output_sorted.squeeze(dim=1), context_vector), dim=1) # (batch_size, hidden_size * (2 + isBiDirectional))
            
            decoded_outputs_sorted:list = [self.outputDenseLayer(dense_layer_input)]
            ...
            for sequence_index in range(Y_Ground_Truth.shape[1] - 1): # Note: -1 to remove EOS token at end of sequence
                ...
                decoded_outputs_sorted.append(self.outputDenseLayer(dense_layer_input))
        ...
        #     "NOT SUPPORTED"
            ...
            # rnn_out.shape = (batch_size=0, seq_length=0, hidden_size * (1 + isBiDirectional))
            ...
            decoder_output, last_decoder_hidden_state = self.decoder(X=first_input, Input_Hidden_State=initial_decoder_hidden_state)
            ...
            for sequence_index in range(Y_Ground_Truth.shape[0] - 1): # Note: -1 to remove EOS token at end of sequence
                ...
                decoded_outputs.append(self.outputDenseLayer(dense_layer_input))
            ...
            decoder_output, last_decoder_hidden_state = self.decoder(first_input, initial_decoder_hidden_state)
...
# out = model(X, Y, X_sequence_lengths)
