...
class TransformerModel(nn.Module):
    def __init__(self):
        ...
        self.encoder_embedding:nn.Embedding = nn.Embedding(num_embeddings=NUM_FRENCH_WORDS, embedding_dim=EMBEDDING_SIZE, padding_idx=PADDING_TOKEN)
        self.decoder_embedding:nn.Embedding = nn.Embedding(num_embeddings=NUM_ENGLISH_WORDS, embedding_dim=EMBEDDING_SIZE, padding_idx=PADDING_TOKEN)
        ...
        self.fc_out:nn.Linear = nn.Linear(in_features=EMBEDDING_SIZE, out_features=NUM_ENGLISH_WORDS)
...
for idx in single_X[0]:
    if idx.item() != PADDING_TOKEN:
        input_sequence.append(ix_to_french_word.get(idx.item(), "<UNK>"))

# Convert the predicted sequence to French words
predicted_sequence = []
for idx in predicted_indices:
    if idx.item() != PADDING_TOKEN:
        predicted_sequence.append(ix_to_english_word.get(idx.item(), "<UNK>"))

# Convert the expected sequence to French words
expected_sequence = []
for idx in single_Y[0]:
    if idx.item() != PADDING_TOKEN:
        expected_sequence.append(ix_to_english_word.get(idx.item(), "<UNK>"))

# Print the results
print("\nInput (French):", " ".join(input_sequence))
print("\nPredicted (English):", " ".join(predicted_sequence))
print("\nExpected (English):", " ".join(expected_sequence))
