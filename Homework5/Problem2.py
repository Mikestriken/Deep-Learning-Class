...
for i in range(len(encoded_dataset) - SEQUENCE_LENGTH):
    sequence = encoded_dataset[i:i + SEQUENCE_LENGTH]
    label = encoded_dataset[i + SEQUENCE_LENGTH]
    X.append(sequence)
    y.append(label)
...
with torch.inference_mode():
    ...
    X_test_batch, Y_test_batch = firstBatch
    ...
    inferenceTime:float = time.time() - startTime
    
    outputIndex = 2
    
    inputSentence:str = ''.join(ix_to_char[i] for i in X_test_batch[outputIndex].cpu().tolist())
    print(f"Inference Time: {(inferenceTime * 1000):.6f} ms")
    print(f"Output Logit: {modelLogits[outputIndex]}")
    print(f"Input Sentence: '{inputSentence}'")
    print(f"Expected Label: {torch.argmax(modelLogits[outputIndex]).cpu().item()} => {ix_to_char[torch.argmax(modelLogits[outputIndex]).cpu().item()]}")
    print(f"Actual Label: {Y_test_batch[outputIndex].cpu().item()} => {ix_to_char[Y_test_batch[outputIndex].cpu().item()]}\n\n")
